...
const ProductCategorySection = ({ section, handleAddToCart, cart }) => {
  return (
    <div>
      <h1>
      ...
      <div className="product-list">
        {section.plants.map((p, idx) => (
          <ProductGridItem
            key={idx}
            item={p}
            handleAddToCart={handleAddToCart}
            isInCart={cart.find((itm) => itm.name === p.name)}
          />
        ))}
      </div>
    </div>
...
const ProductGridItem = ({ item, handleAddToCart, isInCart }) => {
  return (
